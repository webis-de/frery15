...
def may_download_data(url, prefix_dir, dir):
...
def may_unzip_corpus(dir_zips, data_dir, corpora_dir):
    for _, _, files in os.walk(dir_zips):
        for file in files:
            if file.endswith(".zip") and not os.path.exists(data_dir + '/' + corpora_dir + '/' + file[:-4]):
                with zipfile.ZipFile(data_dir + '/' + corpora_dir + '/' + file, "r") as z:
                    z.extractall(data_dir + '/' + corpora_dir + '/')

                # Unzipped file has an other name than zip file
                # assert os.path.exists(data_dir+'/'+train_corpora_dir+'/'+file[:-4])
                os.remove(data_dir + '/' + corpora_dir + '/' + file)
